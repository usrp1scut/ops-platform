# API-only image. ops-api no longer embeds or serves any frontend — the
# React/Vite SPA is built and served entirely by the standalone web/ image
# (nginx), which reverse-proxies /api, /auth, /healthz and /ws back to this
# binary on the same origin. See web/Dockerfile and docker-compose.yml.
